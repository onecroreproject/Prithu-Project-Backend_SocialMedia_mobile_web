...
async function runTest() {
    ...
    const payModes = ["cc", "dc", "upi", "nb", "all"];
    for (const payMode of payModes) {
        ...
        console.log(`Sending CreateOrder with payMode=${payMode}...`);
        ...
        try {
            ...
            console.log(`SUCCESS for payMode=${payMode}:`, res.data);
            return;
        } catch (error) {
            console.log(`ERROR for payMode=${payMode}: STATUS=`, error.response?.status, "DATA=", error.response?.data);
        }
    }
}
