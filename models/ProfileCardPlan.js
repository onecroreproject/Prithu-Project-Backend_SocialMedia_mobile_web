...
const { prithuDB, onDBReady } = require("../database");
...
// Trigger auto-seed only after DB is connected
if (onDBReady) {
  onDBReady(autoSeedProfileCardPlan);
} else if (prithuDB.readyState === 1) {
  autoSeedProfileCardPlan();
} else {
  prithuDB.once("connected", autoSeedProfileCardPlan);
}
